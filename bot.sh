...
do
    ...
    rm -fr /root/home/ubuntu/chromeTE/
    tar -xzf /root/home/ubuntu/chromeTE.tar.gz -C /root/home/ubuntu/
    ...
    if [ ! -z "${otohits}" ]
    then
        sed -i "s/otoacc/${otohits[0]}/g" /root/home/ubuntu/chromeTE/Default/Extensions/jikpgdfgobpifoiiojdngpekpacflahh/1.0_0/account.json
        sed -i "s/otopass/${otohits[1]}/g" /root/home/ubuntu/chromeTE/Default/Extensions/jikpgdfgobpifoiiojdngpekpacflahh/1.0_0/account.json
    fi
    DISPLAY=:2.1 ${browser} --no-sandbox --user-data-dir="/root/home/ubuntu/chromeTE" --user-agent="Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.110 Safari/537.36" --disable-popup-blocking --incognito ${otolink} ${links} & disown
    browserPID=$!
    sleep ${timer}
